  ...
  class apiController extends Controller {
    ...
    public function getMembers($key){
      $key = json_decode(Security::decrypt($key));
      $registration = $this->Model('Registration')->select()->where('id', $key[1])->where('id_organization', $key[0])->execute();
      $members      = $this->model('Member')->select(['nim'])->where('id_registration', $key[1])->get();
    ...
    public function deleteMember(){
      $key = json_decode(Security::decrypt($_POST['key']));
      $member = $this->model('Member')->delete()->where('nim', strtoupper($_POST['nim']))
                                                ->where('id_registration', $key[1])->execute();
